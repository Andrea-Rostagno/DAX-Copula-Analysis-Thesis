...
plt.savefig('uniform_volatility.png')
...
def neg_gaussian_loglik(rho, u, v): return -gaussian_copula_loglik(rho[0], u, v)


def neg_t_loglik(params, u, v): return -t_copula_loglik(params, u, v)


def neg_clayton_loglik(theta, u, v): return -clayton_copula_loglik(theta[0], u, v)


def neg_gumbel_loglik(theta, u, v): return -gumbel_copula_loglik(theta[0], u, v)


def neg_frank_loglik(theta, u, v): return -frank_copula_loglik(theta[0], u, v)
...
plt.tight_layout()
plt.savefig('simulated_volatility.png')
...
try:
    ...
    # Genera date future per la visualizzazione
    future_dates = pd.date_range(start=returns.index[-1], periods=len(vol_forecast1) + 1)[1:]

    # Visualizzazione delle previsioni di volatilità
    plt.figure(figsize=(12, 6))

    # Dati storici
    plt.plot(vol_returns.index[-60:], vol_returns['vol1'][-60:],
             label=f'{col1} Volatilità storica', color='blue', alpha=0.7)
    plt.plot(vol_returns.index[-60:], vol_returns['vol2'][-60:],
             label=f'{col2} Volatilità storica', color='green', alpha=0.7)

    # Previsioni
    plt.plot(future_dates, annual_vol_forecast1, '--',
             label=f'{col1} Previsione', color='blue', linewidth=2)
    plt.plot(future_dates, annual_vol_forecast2, '--',
    ...
    plt.plot(future_dates, port_vol_forecast, '-',
    ...
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.savefig('portfolio_volatility_forecast.png')
